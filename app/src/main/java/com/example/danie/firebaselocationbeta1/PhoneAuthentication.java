...
public class PhoneAuthentication extends AppCompatActivity {
    ...
    public void signOut(){
        ...
        mAuth= FirebaseAuth.getInstance();
        mAuth.signOut();
        // deleting stored phone number from shared preferences
        deleteStoredAuthenticatedNumber();
        // deleting stored unique id from shared preferences
        deleteStoredUniqueId();
    }
    ...
    public void deleteStoredUniqueId(){
        // deleting the unique id in a shared preferences
        sharedPreferences.edit().remove("uniqueId").apply();
    }
    public void storeAuthenticatedNumber(String phoneNumber){
        ...
        sharedPreferences.edit().putString("authenticatedId",phoneNumber).apply();
        Log.i(TAG,"Authenticated Number has been stored");
    }
    public void storeUniqueId(String uniqueId){
        // storing the unique id generated by the data base for the user
        sharedPreferences.edit().putString("uniqueId",uniqueId).apply();
        Log.i(TAG,"Unique Id has been stored");
    }
    ...
    public String getUniqueId(){
        // get the unique id that is stored in shared preferences
            ...
            public void onComplete(@NonNull Task<QuerySnapshot> task) {
                if (task.isSuccessful()){
                    ...
                    for (DocumentSnapshot document : task.getResult()) {
                        ...
                        users.add(data1).addOnCompleteListener(new OnCompleteListener<DocumentReference>() {
                            @Override
                            public void onComplete(@NonNull Task<DocumentReference> task) {
                                // storing the unique id onto the phone
                                String uniqueId=task.getResult().getId();
                                Log.i(TAG,uniqueId);
                                storeUniqueId(uniqueId);
                                // creating an interaction data for user after all the detail has been gathered
                                // is there a way to this instead in main activity??
                                Interaction interaction=new Interaction(phoneNumber,uniqueId,context);
